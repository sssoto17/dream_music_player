...
export function PasswordResetRequestForm({ dict }) {
	...
	return (
		<>
			<Form action={submit}>
				{state?.success && (
					<p className="col-span-full font-semibold text-emerald-600">
						{state?.success}
					</p>
				)}
				{state?.error && (
					<p className="col-span-full font-semibold text-rose-600">
						{state?.error}
					</p>
				)}
				<FormInput
...
export function PasswordResetForm() {
	...
	const submitReset = ResetAccount.bind(null, key);
